...
{ 
	//initially storing first as index of smallest value
	int index=first;
	//looping from next index to last
	for(int i=first+1;i<=last;i++){
		//checking if item at i is smaller than item at index
		if(list[i]<list[index]){
			//setting i as new index
			index=i;
		}
	}
	//returning index of smallest value in the given range.
	return index;
}
